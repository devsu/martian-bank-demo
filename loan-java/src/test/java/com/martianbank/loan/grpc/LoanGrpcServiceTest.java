...
class LoanGrpcServiceTest {
    ...
    @Mock
    StreamObserver<LoanProto.LoanResponse> loanResponseObserver;

    @Mock
    StreamObserver<LoanProto.LoansHistoryResponse> historyResponseObserver;

    @InjectMocks
    LoanGrpcService loanGrpcService;

    private LoanProto.LoanRequest validGrpcRequest;

    @BeforeEach
    void setUp() {
        validGrpcRequest = LoanProto.LoanRequest.newBuilder()
            .setName("John Doe")
    ...
    void processLoanRequest_returnsApprovedResponse() {
        ...
        ArgumentCaptor<LoanProto.LoanResponse> responseCaptor = ArgumentCaptor.forClass(LoanProto.LoanResponse.class);
        verify(loanResponseObserver).onNext(responseCaptor.capture());
        verify(loanResponseObserver).onCompleted();

        LoanProto.LoanResponse response = responseCaptor.getValue();
    ...
    void getLoanHistory_returnsLoansInResponse() {
        ...
        LoanProto.LoansHistoryRequest request = LoanProto.LoansHistoryRequest.newBuilder()
            .setEmail("john@test.com")
            .build();

        loanGrpcService.getLoanHistory(request, historyResponseObserver);

        ArgumentCaptor<LoanProto.LoansHistoryResponse> responseCaptor = ArgumentCaptor.forClass(LoanProto.LoansHistoryResponse.class);
        verify(historyResponseObserver).onNext(responseCaptor.capture());
        verify(historyResponseObserver).onCompleted();

        LoanProto.LoansHistoryResponse response = responseCaptor.getValue();
        assertEquals(1, response.getLoansCount());

        LoanProto.Loan grpcLoan = response.getLoans(0);
    ...
    void getLoanHistory_returnsMultipleLoans() {
        ...
        loan2.setAccountType("checking");
        loan2.setAccountNumber("67890");
        loan2.setGovtIdType("passport");
        loan2.setGovtIdNumber("ABC123");
        loan2.setLoanType("auto");
        loan2.setLoanAmount(10000.0);
        loan2.setInterestRate(4.5);
        loan2.setTimePeriod("24 months");
    ...
    void getLoanHistory_mapsAllLoanFieldsToGrpc() {
        ...
        LoanProto.Loan grpcLoan = responseCaptor.getValue().getLoans(0);
